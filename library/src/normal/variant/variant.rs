...
impl<AnnotatedT> Variant<AnnotatedT> {
    ...
    pub fn traverse<'keys, IteratorT>(&self, keys: IteratorT) -> Option<&Self>
    where
        AnnotatedT: 'keys,
        IteratorT: Iterator<Item = &'keys Self>,
    {
        let mut found = self;
        for key in keys {
            found = found.get(key)?;
    ...
    /// be more efficient because it doesn't require an allocated iterator.
    pub fn traverse_mut<'keys, IteratorT>(&mut self, keys: IteratorT) -> Option<&mut Self>
    where
        AnnotatedT: 'keys,
        IteratorT: Iterator<Item = &'keys Self>,
    {
    ...
    ///
    /// Note that the iterator is a `dyn` in order to support different underlying implementations.
    pub fn key_value_iterator(&self) -> Option<Box<dyn KeyValuePairIterator<AnnotatedT> + '_>>
    where
        AnnotatedT: Default,
    {
        match self {
    ...
    pub fn annotated_depiction(&self) -> AnnotatedVariantDepiction<'_, AnnotatedT> {
