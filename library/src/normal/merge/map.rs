...
impl<AnnotatedT> Map<AnnotatedT> {
    ...
    fn merge_key<ProblemReceiverT>(
        &mut self,
        other_key: &Variant<AnnotatedT>,
        other_value: &Variant<AnnotatedT>,
        merge_mode: &MergeMode,
        problems: &mut ProblemReceiverT,
    ) -> Result<bool, Problem>
    where
            ...
            None => {
                ...
                if self.inner.insert(other_key.clone(), other_value.clone()).is_some() {
                    if merge_mode.map == MapMergeMode::FailExisting {
                        problems.give(MergingError::as_problem("map").maybe_with(other_key.annotations().cloned()))?;
                    }
                }
                Ok(true)
            }
