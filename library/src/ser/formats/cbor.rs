...
impl Serializer {
        ...
        SerializableT: Serialize + ?Sized,
    {
        ...
        if self.base64 {
            write(value, &mut Self::base64_writer(writer)).into_low_level_serialize_problem(Format::CBOR)?;
        } else {
            write(value, writer).into_low_level_serialize_problem(Format::CBOR)?;
        }

        if self.pretty { Self::write_newline(writer).into_low_level_serialize_problem(Format::CBOR) } else { Ok(()) }
    }
}
