...
impl Serializer {
        ...
        SerializableT: Serialize + ?Sized,
    {
        ...
        json_stream_writer.serialize_value(&value).via(SerializeError::new("serde")).with(Format::JSON)?;
        json_stream_writer.finish_document().via(SerializeError::new("serde")).with(Format::JSON)?;

        if self.pretty { Self::write_newline(writer).into_low_level_serialize_problem(Format::JSON) } else { Ok(()) }
    }
}
