...
where
    ...
    AnnotatedT: Annotated + Clone + Default,
{
    let event = decoder.next_event().into_parse_problem(Format::CBOR)?;
    trace!("{:?}", event);
    ...
    match event {
        ...
        Event::Signed(integer) => {
            let integer = Event::interpret_signed_checked(integer)
                ...
                .into_parse_problem(Format::CBOR)?;
        ...
        Event::UnknownLengthTextString => {
            let string = read_cbor_unknown_length_text_string(decoder).into_parse_problem(Format::CBOR)?;
            value_builder.add(Text::from(string).with_label(label), None);
        }
        ...
        Event::UnknownLengthByteString => {
            let bytes = read_cbor_unknown_length_bytes(decoder).into_parse_problem(Format::CBOR)?;
            value_builder.add(Blob::from(bytes).with_label(label), None);
        }
        ...
        Event::UnknownLengthArray => {
            value_builder.start_list_with_label(label, None);
            loop {
                match decoder.next_event().into_parse_problem(Format::CBOR)? {
                    Event::Break => {
                        break;
                    }
