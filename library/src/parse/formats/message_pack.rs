...
where
    ...
    AnnotatedT: Annotated + Clone + Default,
{
    let marker = read_marker(reader).map_err(|error| error.0).into_parsing_problem(Format::MessagePack)?;
    trace!("{:?}", marker);
    match marker {
        Marker::Reserved => {}
        ...
        Marker::I8 => {
            value_builder.add(Integer::from(read_i8(reader).into_parsing_problem(Format::MessagePack)? as i64), None)
        }
        Marker::I16 => {
            value_builder.add(Integer::from(read_i16(reader).into_parsing_problem(Format::MessagePack)? as i64), None)
        }
        Marker::I32 => {
            value_builder.add(Integer::from(read_i32(reader).into_parsing_problem(Format::MessagePack)? as i64), None)
        }
        Marker::I64 => {
            value_builder.add(Integer::from(read_i64(reader).into_parsing_problem(Format::MessagePack)?), None)
        }
        Marker::FixPos(integer) => value_builder.add(UnsignedInteger::from(integer as u64), None),
        Marker::U8 => value_builder
            .add(UnsignedInteger::from(read_u8(reader).into_parsing_problem(Format::MessagePack)? as u64), None),
        Marker::U16 => value_builder
            .add(UnsignedInteger::from(read_u16(reader).into_parsing_problem(Format::MessagePack)? as u64), None),
        Marker::U32 => value_builder
            .add(UnsignedInteger::from(read_u32(reader).into_parsing_problem(Format::MessagePack)? as u64), None),
        Marker::U64 => {
            value_builder.add(UnsignedInteger::from(read_u64(reader).into_parsing_problem(Format::MessagePack)?), None)
        }
        Marker::F32 => value_builder.add(Float::from(read_f32(reader).into_parsing_problem(Format::MessagePack)?), None),
        Marker::F64 => value_builder.add(Float::from(read_f64(reader).into_parsing_problem(Format::MessagePack)?), None),

        Marker::Bin8 => {
            let length = read_u8(reader).into_parsing_problem(Format::MessagePack)? as usize;
            read_message_pack_bytes(reader, value_builder, length)?;
        }

        Marker::Bin16 => {
            let length = read_u16(reader).into_parsing_problem(Format::MessagePack)? as usize;
            read_message_pack_bytes(reader, value_builder, length)?;
        }

        Marker::Bin32 => {
            let length = read_u32(reader).into_parsing_problem(Format::MessagePack)? as usize;
            read_message_pack_bytes(reader, value_builder, length)?;
        }
        ...
        Marker::FixExt1 => {
            let label = read_i8(reader).into_parsing_problem(Format::MessagePack)? as i64;
    ...
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    reader.read_exact_buf(&mut buffer).into_parsing_problem(Format::MessagePack)?;
    let string = String::from_utf8(buffer).into_parsing_problem(Format::MessagePack)?;
