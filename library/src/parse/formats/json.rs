    ...
    AnnotatedT: Annotated + Clone + Default,
{
    ...
    let value = reader.peek().into_parse_problem(Format::JSON)?;
    tracing::trace!("{}", value);
    match value {
        ValueType::Null => {
            let span = get_span(reader);
            reader.next_null().into_parse_problem(Format::JSON)?;
        ...
        ValueType::Number => {
            if try_integers || try_unsigned_integers {
                let span = get_span(reader);
                let number = reader.next_number_as_str().into_parse_problem(Format::JSON)?;
                if let Some(number) = if try_unsigned_integers { number.parse::<u64>().ok() } else { None } {
                    ...
                    value_builder.add(
                        Float::from(number.parse::<f64>().into_parse_problem(Format::JSON)?).with_span(span),
                        None,
                    );
                }
            } else {
                let span = get_span(reader);
                let number: f64 = reader.next_number().into_parse_problem(Format::JSON)??;
        ...
        ValueType::Boolean => {
            let span = get_span(reader);
            value_builder
                .add(Boolean::from(reader.next_bool().into_parse_problem(Format::JSON)?).with_span(span), None);
        }

        ValueType::String => {
            let span = get_span(reader);
            value_builder.add(
                Text::from(ByteString::from(reader.next_str().into_parse_problem(Format::JSON)?)).with_span(span),
                None,
            );
        }
        ...
        ValueType::Array => {
            let span = get_span(reader);
            reader.begin_array().into_parse_problem(Format::JSON)?;
            value_builder.start_list_with_span(span, None);
            while reader.has_next().into_parse_problem(Format::JSON)? {
                read_next_json(reader, value_builder, hints, try_integers, try_unsigned_integers)?;
            }
            value_builder.end_container();
            reader.end_array().into_parse_problem(Format::JSON)?;
        }

        ValueType::Object => {
            let span = get_span(reader);
            reader.begin_object().into_parse_problem(Format::JSON)?;
            value_builder.start_map_with_span(span, None);
            while reader.has_next().into_parse_problem(Format::JSON)? {
                // Key
                let span = get_span(reader);
                value_builder.add(
                    Text::from(ByteString::from(reader.next_name().into_parse_problem(Format::JSON)?)).with_span(span),
                    None,
                );
            ...
            reader.end_object().into_parse_problem(Format::JSON)?;
        }
    }
