...
impl Parser {
        ...
        AnnotatedT: Annotated + Clone + Default,
    {
        ...
        SaphyrParser::new_from_iter(io::BufReader::new(reader).chars())
            ...
            .into_parse_problem(Format::YAML)?;
    ...
    AnnotatedT: Annotated,
{
    ...
    fn parse_yaml_null(&self, value: &str, span: &SaphyrSpan) -> Result<(), Problem> {
        if self.allow_legacy_words {
            ...
            match value {
                "~" | "null" | "Null" | "NULL" => Ok(()),
                _ => Err(ScanError::new_str(span.start, "not a null").into_parse_problem(Format::YAML)),
            }
        } else {
            // Core schema, https://yaml.org/spec/1.2.2/#1032-tag-resolution
    ...
    fn parse_yaml_bool(&self, value: &str, span: &SaphyrSpan) -> Result<bool, Problem> {
        if self.allow_legacy_words {
            ...
            match value {
                ...
                _ => Err(ScanError::new_str(span.start, "not a bool").into_parse_problem(Format::YAML)),
            }
        } else {
            // Core schema, https://yaml.org/spec/1.2.2/#1032-tag-resolution
    ...
    fn parse_yaml_integer(value: &str, span: &SaphyrSpan) -> Result<i64, Problem> {
        ...
        value.parse().map_err(|_| ScanError::new_str(span.start, "not an integer").into_parse_problem(Format::YAML))
    }
    ...
    fn parse_yaml_float(value: &str, span: &SaphyrSpan) -> Result<f64, Problem> {
        ...
        match value {
            ...
            _ => value
                .parse()
                .map_err(|_| ScanError::new_str(span.start, "not a float").into_parse_problem(Format::YAML)),
        }
    }
}

impl<'this, 'input, AnnotatedT> SpannedEventReceiver<'input> for YamlReceiver<AnnotatedT>
where
    AnnotatedT: Annotated + Clone + Default,
{
