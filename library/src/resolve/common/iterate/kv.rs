...
pub struct ResolvingKeyValuePairIterator<'inner, AnnotatedT> {
    /// Inner key-value pair iterator.
    pub inner: Box<dyn IntoKeyValuePairIterator<AnnotatedT> + 'inner>,
}

impl<'inner, AnnotatedT> ResolvingKeyValuePairIterator<'inner, AnnotatedT> {
    /// Constructor.
    pub fn new(inner: Box<dyn IntoKeyValuePairIterator<AnnotatedT> + 'inner>) -> Self {
        Self { inner }
    }
    ...
    where
        AnnotatedT: 'inner + Annotated + Clone + Default,
        ProblemReceiverT: ProblemReceiver,
    {
        if variant.is_collection() {
            ...
            problems
                .give(IncompatibleVariantTypeError::as_problem_from(&variant, &["map", "list"]).via(ResolveError))?;
            Ok(None)
        }
    }
}

impl<'inner, KeyT, ValueT, AnnotatedT> ResolvingIterator<(KeyT, ValueT)>
    for ResolvingKeyValuePairIterator<'inner, AnnotatedT>
where
        ...
        ProblemReceiverT: ProblemReceiver,
    {
        ...
        loop {
            match self.inner.next() {
                ...
                Err((problem, cause)) => {
                    problems.give(problem.maybe_with(cause.annotations().cloned()).via(ResolveError))?
                }
            }
        }
    }
